...
Active build. Implemented and tested so far: the Avalanche contract + EIP-712 cooperative-validation logic (local-EVM test suite green), the message/bus core, and the project spine. In progress: simulator world + fleet, hybrid render + CV defect detection + SAR, graph-theoretic coordination, the dashboard, Fuji deployment, and the full end-to-end run.

License: **GPL-3.0-or-later**. Derives from COOP-UAV-S (GPL-3.0). The bundled 3D infrastructure model is third-party — see `assets/model/README.md` for its own license and attribution.
